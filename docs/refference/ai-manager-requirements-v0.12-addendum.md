...
- DM 送信は「対話レコード作成(SoT)→送信→失敗時は補償削除」の adhoc-checkin と同じパターン。
  送信できなければエスカレーションは open のまま残す(解決済みなのに未回答、という不整合を作らない)
...
- 集計は新ジョブ `daily-etl` が `dwh.run_daily_etl()` を実行する。手動実行の既定対象日は
  **当日**(「今日の途中経過を今すぐ集計に反映する」が目的。対象日のファクトは洗い替えで冪等、
...
- 回答に不備がある対話への**フィードバック**: 管理者が正しい回答・指摘を入力すると、
  ...
  4. 送達で status='delivered'。**送信失敗時は pending のまま残り、画面の「再送」で回復できる**
     (手動回復パス — 原則 6。二重配信防止のため delivered の再送は拒否)
