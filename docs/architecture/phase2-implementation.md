...
- **整合の担保**: 「メンバーへ回答送信」は、解決の記録(open→resolved)を**送信前にアトミックに
  クレーム**することで並行操作(ダブルクリック・複数管理者の同時操作・Chat 裁定との交差)の
  二重 DM 送信を防ぐ(クレームに敗れた側は DM を送らずスキップ)。その後 adhoc-checkin と同じ
  「対話レコード作成(SoT)→ DM 送信」を行い、送信失敗時は補償(対話レコード削除+
  エスカレーションを open へ戻す)で再操作可能にする(解決済みなのに未回答という不整合を
  作らない)。クレーム後〜送信前のプロセス断で「resolved だが未送達」となる残余ウィンドウは
  数百 ms 規模の既知の制約として許容(コードコメントに明記)。還流可否の判定は
  shared の isRefluxableResolutionType に一元化し、Chat の再還流ボタン・batch・dashboard の
  3消費者で条件をドリフトさせない
...
- **回復経路**: 訂正 DM の送信失敗時はフィードバックが pending のまま残り、画面の「再送」で
  ...
  (二重配信防止・冪等)。配信は送信前に pending→delivered をアトミックにクレームし、
  並行起動の二重配信を防ぐ(送信失敗時は補償で pending へ戻す — ADR-18 と同じ設計)。
  送達済みで還流のみ未了の場合は、画面の「ナレッジ再還流」(refluxOnly)で DM を送らずに
  還流だけを再試行できる(SoT → キャッシュ同期パスの恒久欠落を防ぐ手動回復パス)
