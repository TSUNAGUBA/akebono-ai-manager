...
| AIM-4003 | Embedding 失敗 | `EMBEDDING_MODEL` / `EMBEDDING_DIMENSIONS`(rag スキーマの vector(768) と一致)を確認。HTTP 404 はモデルが呼び出し先ロケーションで未提供 — `VERTEX_EMBEDDING_LOCATION` の提供状況を確認(deployment-setup.md トラブルシューティング参照) |
...
| AIM-5005 | ジョブのパラメータ不正 | 対象(エスカレーション・対話・フィードバック)が存在するか、必須項目・文字数上限、操作者が active な管理者かを確認。DM 未登録(chat_space_id なし)のメンバー宛て送信もこのコード |
| AIM-5006 | 集計 ETL(dwh.run_daily_etl)の実行失敗 | db-migrate の適用状況(SECURITY DEFINER 化を含む 20_daily_etl.sql / 30_grants.sql)と、ログの cause の SQL エラーを確認 |
...
| AIM-6006 | ナレッジ管理の Drive 書込失敗 | HTTP 403/404 はナレッジフォルダのランタイム SA への共有が「閲覧者」のまま(または未共有・フォルダ ID 誤り)。**編集者**で共有し直す(deployment-setup.md Step 7-3)。その他はログの `details` を確認 |
| AIM-6007 | 今すぐ同期(knowledge-sync)の起動失敗 | batch サービスの状態と、`BATCH_URL` の配線(デプロイログ)・ランタイム SA の `roles/run.invoker` を確認。タイムアウトの場合はジョブが継続している可能性があるため、しばらく後にページを再読み込みして同期状態を確認 |
| AIM-6008 | 状況確認(adhoc-checkin)の起動失敗 | AIM-6007 と同じ確認(batch サービスの状態・`BATCH_URL` の配線・`roles/run.invoker`)。タイムアウトの場合は配信が継続している可能性があるため、しばらく後に /admin/checkin を再読み込みして当日の状況列を確認 |
| AIM-6009 | エスカレーション解決アクション(escalation-action)の起動失敗 | AIM-6007 と同じ確認。対象・入力起因の失敗は画面のメッセージ(batch 側 AIM-5005)を確認。タイムアウトの場合は処理が継続している可能性があるため /admin/escalations を再読み込みして状態を確認 |
| AIM-6010 | 対話フィードバック(dialogue-feedback)の起動失敗 | AIM-6007 と同じ確認。フィードバックが「訂正送信待ち」のまま残っている場合は /admin/dialogues の再送で回復できる |
| AIM-6011 | ジョブ実行ページからの手動起動失敗 | AIM-6007 と同じ確認。いずれのジョブも再実行安全のため、復旧後にもう一度実行してよい |
