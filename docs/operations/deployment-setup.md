...
3. **Drive ナレッジフォルダの共有**: ナレッジフォルダをランタイム SA(`ai-manager-runtime@<project>.iam.gserviceaccount.com`)に**編集者**で共有し、フォルダ ID(フォルダを開いた URL の末尾)を `KNOWLEDGE_DRIVE_FOLDER_ID` に登録(編集者が必要なのは v0.4 のナレッジ管理 UI(Step 7-8)が文書の投入・削除を書き込むため。閲覧のみの共有(閲覧者)でも同期と一覧は動くが、UI からの投入・削除は権限エラー(AIM-6006)になる)
   ...
   - **同期対象の形式**: `.md` / `.txt` / Google ドキュメント(テキスト抽出)。PDF・Word(.docx)等はスキップされる(一覧には表示される)
...
| ナレッジ管理の一覧が「アクセスできません」/「今すぐ同期」が失敗(AIM-5003) | `KNOWLEDGE_DRIVE_FOLDER_ID` の ID 誤り、またはナレッジフォルダがランタイム SA に未共有。フォルダ URL 末尾の ID を登録し、SA に**編集者**で共有する(Step 7-3)。※過去の版では未共有でもエラーにならず「一覧 0 件・同期 0 件」と表示されていた |
| ナレッジを置いたのに検索に反映されない/顧客スコープが付かない | 同期対象は `.md` / `.txt` / Google ドキュメントのみ(PDF・Word はスキップ)。顧客固有の文書は `customer/{顧客ID}/` 配下に置き、フォルダ名を顧客マスタの顧客ID と一致させる(Step 7-3 の規約) |
