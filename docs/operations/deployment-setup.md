...
| ナレッジが同期されない(同期しても検索に出ない・0件) | ①ナレッジ管理ページの一覧に対象ファイルが**表示されているか**確認(表示されないファイルは同期されない)②「同期対象フォルダ: Drive で開く」リンクの遷移先が意図したフォルダか確認(`KNOWLEDGE_DRIVE_FOLDER_ID` のずれ)③ページ上部に**ショートカット警告**が出ていないか確認 — ショートカットは共有を引き継がないため、実体フォルダの移動または実体側の SA 共有が必要(Step 7-3・v0.11)④スキャン画像のみの PDF はテキスト層がなく同期スキップされる(同期ログの警告を確認) |
