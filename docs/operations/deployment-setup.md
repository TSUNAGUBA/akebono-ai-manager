...
3. **Drive ナレッジフォルダの共有**: ナレッジフォルダをランタイム SA(`ai-manager-runtime@<project>.iam.gserviceaccount.com`)に**編集者**で共有し、フォルダ ID(フォルダを開いた URL の末尾)を `KNOWLEDGE_DRIVE_FOLDER_ID` に登録(編集者が必要なのは v0.4 のナレッジ管理 UI(Step 7-8)が文書の投入・削除を書き込むため。閲覧のみの共有(閲覧者)でも同期と一覧は動くが、UI からの投入・削除は権限エラー(AIM-6006)になる)
   - **フォルダ構成の規約(M1)**: ルート直下に `judgement/`(共通の判断基準・例え話)、`domain/{業界ID}/`(業界知識)、`customer/{顧客ID}/`(顧客固有)を置く。顧客フォルダ名は**顧客マスタの顧客ID と一致**させる(例: `customer/shimamura/`)。規約外の場所(ルート直下の任意フォルダ等)の文書も取り込まれるが、業界・顧客スコープが付かず「共通の業務知識」として扱われる
   - **同期対象の形式**: `.md` / `.txt` などのテキスト系ファイル(MIME が text/* のもの+拡張子 .md)と Google ドキュメント(テキスト抽出)。PDF・Word(.docx)等はスキップされる(一覧には表示される)
...
8. **ナレッジ管理 UI**(v0.4 — ナレッジ文書の一覧・投入・上書き・削除・即時同期をダッシュボードから行う):
   ...
   - 投入は直接入力に加えて**ファイルアップロード(複数選択可)**でも行える(v0.6 — .md / .txt、
     1ファイル 200KB・最大 10 件。ファイル名は小文字に変換して保存され、規約外・UTF-8 以外・重複が
     1件でもあると全件投入されない。追加の設定は不要)
...
| ナレッジ管理の一覧に「ナレッジフォルダの一覧を取得できませんでした」/「今すぐ同期」が失敗する | `KNOWLEDGE_DRIVE_FOLDER_ID` の ID 誤り(ファイル ID・ゴミ箱内フォルダを含む)、またはフォルダがランタイム SA に未共有。フォルダ URL 末尾の ID を登録し、SA に**編集者**で共有する(Step 7-3)。同期失敗の画面表示は AIM-6007 で、原因の詳細は batch ログの AIM-5003 に出る。※過去の版では未共有でもエラーにならず「一覧 0 件・同期 0 件」と表示されていた |
| ナレッジを置いたのに検索に反映されない/顧客スコープが付かない | 同期対象は `.md` / `.txt` などのテキスト系と Google ドキュメント(PDF・Word はスキップ)。顧客固有の文書は `customer/{顧客ID}/` 配下に置き、フォルダ名を顧客マスタの顧客ID と一致させる(Step 7-3 の規約) |
