...
| `ai-manager-db-master-admin-user` / `-password` | ai_manager_admin_rw(マスタ管理・ユーザー設定 UI 用) | dashboard(`DASHBOARD_ADMIN_DB_ENABLED=true` 時のみ) |
...
| `CALENDAR_ENABLED` | — | `true` で朝の問いかけに本人の当日予定を反映+随時 QA が予定質問に本人のカレンダーで回答(v0.14)(要 Workspace のドメイン全体委任。Step 7-6) |
...
3. **Drive ナレッジフォルダの共有**: ナレッジフォルダをランタイム SA(`ai-manager-runtime@<project>.iam.gserviceaccount.com`)に**編集者**で共有し、フォルダ ID を `KNOWLEDGE_DRIVE_FOLDER_ID` に登録(編集者が必要なのは v0.4 のナレッジ管理 UI(Step 7-8)が文書の投入・削除を書き込むため。閲覧のみの共有(閲覧者)でも同期と一覧は動くが、UI からの投入・削除は権限エラー(AIM-6006)になる)
   - **ショートカットの注意(v0.11)**: フォルダ ID は URL(`https://drive.google.com/drive/folders/<この部分>`)から取得する。既存フォルダを**ショートカット**でナレッジフォルダ内に置いた場合、ショートカットは **Drive の共有権限を引き継がない**ため、ショートカット先(実体)も SA へ共有しないと読めない(推奨は実体フォルダをナレッジフォルダ内へ移動)。アクセスできないショートカットはナレッジ管理ページに警告表示され、同期ログ(AIM-5003 系警告)にも記録される
   - **設定の確認方法**: ダッシュボードの「マスタ管理 → ナレッジ」を開く。①一覧に想定のファイルが表示されるか(表示されないファイルは同期されない)②「同期対象フォルダ: Drive で開く」リンクの遷移先が意図したフォルダか、の2点でフォルダ ID・共有のずれを自己診断できる
...
6. **カレンダー連携の有効化**(Phase 2、任意 — 朝の問いかけに本人の当日予定を反映+随時 QA の予定回答(v0.14)):
   ...
   - 随時 QA の予定回答(「明日の予定は?」等 — v0.14)にも**同じ設定が使われる**(ランタイム SA は
     3 サービス共通のため、追加の設定は不要。デプロイ時に chat-gateway へも自動配線される)
   - 未設定の間は従来どおり(朝の問いかけはタスク状況のみ・随時 QA はカレンダーなし)で動作する
     (取得失敗も同様に非ブロッキング)

7. **マスタ管理 UI の有効化**(Phase 2、任意 — 業界・顧客(エイリアス含む)・顧客間関係・プロジェクト・ユーザーの問いかけ設定をダッシュボードから管理):
...
8. **ナレッジ管理 UI**(v0.4 — ナレッジ文書の一覧・投入・上書き・削除・即時同期をダッシュボードから行う):
   ...
   - 投入は直接入力に加えて**ファイルアップロード(複数選択可)**でも行える(v0.6 — 最大 10 件。
     .md / .txt は 1ファイル 200KB、.pdf は 3MB(v0.11)、1回の送信全体で 4MB まで。
     ファイル名は日本語のままでよく(v0.11 で文字種制限を撤廃)、英大文字のみ小文字に変換して保存される。
     形式・サイズ・文字コード・重複の不正が1件でもあると全件投入されない)
   - **PDF の扱い(v0.11)**: PDF はバイナリのまま Drive へ保存され、同期時にテキスト層を抽出して
     検索対象になる。スキャン画像のみ(テキスト層なし)の PDF は同期時にスキップされ、警告ログに残る
...
| 朝の問いかけが届かない | ①ダッシュボードの「ユーザー」タブで問いかけ不可に設定されている(v0.8: 可否はユーザー単位。既存の管理者は移行時に不可で初期化される — 新規追加時の既定はロール不問で可、v0.8 §2) ②対象ユーザーの `chat_space_id` が未登録(本人が一度 Chat アプリに話しかけると自動登録される) |
| 顧客名を含む質問が「登録されていない」と回答される | 顧客の**登録名が質問文に部分一致しない**(例: 登録名「株式会社しまむら」は質問「しまむらの…」に一致しない — 照合は「質問文 ⊇ 登録名」方向)。顧客編集フォームで通称をエイリアスに登録する(v0.9 §4)。エスカレーション本文の「対象顧客」欄で特定成否を確認できる。特定済みなのに回答できない場合は顧客間関係(/admin/relations)の登録を確認 |
| Chat の返信に「処理中にエラーが発生しました(エラーコード: AIM-xxxx)」 | コードを [error-codes.md](./error-codes.md) で逆引きし、Cloud Logging で該当エラーを確認する(v0.9 §5)。対話継続中の LLM 一時障害は自動で定型文にフォールバックし返信は保存される — 汎用エラーが出るのは保存にも失敗した場合(DB 接続等を確認) |
...
| ナレッジが同期されない(同期しても検索に出ない・0件) | ①ナレッジ管理ページの一覧に対象ファイルが**表示されているか**確認(表示されないファイルは同期されない)②「同期対象フォルダ: Drive で開く」リンクの遷移先が意図したフォルダか確認(`KNOWLEDGE_DRIVE_FOLDER_ID` のずれ)③ページ上部に**ショートカット警告**が出ていないか確認 — ショートカットは共有を引き継がないため、実体フォルダの移動または実体側の SA 共有が必要(Step 7-3・v0.11)。**警告が解消されるまで、Drive から削除した文書の検索除外(チャンク掃除)も保留される**(見えない配下を「削除された」と誤判定しないための保護)④スキャン画像のみの PDF はテキスト層がなく同期スキップされる(同期ログの警告を確認) |
| ナレッジ管理の「今すぐ同期」ボタン/状況確認の送信ボタンが出ない | デプロイ時に batch の URL(`BATCH_URL`)を取得できなかった(デプロイログの warning を確認)。batch が正常なら Deploy の再実行で配線される。ボタンなしでも毎日 06:30 の自動同期・平日 08:00 の朝の問いかけは動作する |
