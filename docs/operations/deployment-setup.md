...
| `DASHBOARD_AUTH_MODE` | — | アプリ層認証 `iap` / `header` / `dev`。未設定時は公開モードから導出(`lb-iap`→`iap`、`direct-iap`→`header`) |
...
4. **ダッシュボードの公開構成**: 2つのモードから選ぶ(`DASHBOARD_EXPOSURE` secret で切替)
   ...
   **A. MVP: Cloud Run 直付け IAP(`direct-iap`)— LB・ドメイン・追加費用なしで `*.run.app` URL を IAP 保護のまま公開**
   ...
   その後 `deploy-config.json` で `DASHBOARD_EXPOSURE` を `direct-iap`、`DASHBOARD_AUTH_MODE` を `header` にして
   `register-github-secrets.ps1` を再実行し、Deploy を再実行する。IAP でログイン済みのユーザーだけが
   `*.run.app` URL を閲覧でき、アプリ層では IAP が付与する認証済みヘッダーでロールを解決する。
   `direct-iap` は `--no-allow-unauthenticated` でデプロイされるため、IAP 有効化前のアクセスは全拒否(安全側)。
...
| ダッシュボードが 401 | `lb-iap`: IAP 経由でアクセスしているか、`DASHBOARD_IAP_AUDIENCE` を確認。`direct-iap`: `DASHBOARD_AUTH_MODE` が `iap` のまま残っていないか(`header` にするか secret を削除) |
