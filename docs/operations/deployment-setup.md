...
7. **マスタ管理 UI の有効化**(Phase 2、任意 — 業界・顧客・顧客間関係をダッシュボードから管理):
   1. DB ロール作成: `create-db-roles.sql` を `-v admin_rw_password='<強いパスワード>'` 付きで再実行(既存ロールはスキップされ冪等)→ db-migrate ジョブ再実行で GRANT 反映
   2. `register-gcp-runtime-secrets.ps1` を再実行し `ai-manager-db-master-admin-*` を登録
