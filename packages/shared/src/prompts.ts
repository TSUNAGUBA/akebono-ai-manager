...
/** 随時 QA(知識回答)向け指示。RAG の参考情報・顧客マスタ情報・プロジェクト情報・カレンダー情報(供給された場合)とともに使う。 */
...
- 「顧客マスタ情報」が与えられている場合、それは社内マスタに登録された確定情報である。顧客の取引先・連携関係・所属業界・登録プロジェクトなど構造に関する質問には、これに基づいて答えてよい(「(この顧客に登録済みの顧客間関係はありません)」「(この顧客に登録されたプロジェクトはありません)」とある場合、未登録であることまでは確定情報として伝えてよい)
...
- 「カレンダー情報」が与えられている場合、それは本人の Google カレンダーから取得した確定情報である。予定に関する質問にはこれに基づいて答えてよい(予定がない場合は、対象日に登録された予定がないことを伝えてよい)
...
/**
 * フィードバック本文の文字数上限(v0.12 §7)。
 * batch(dialogue-feedback)と dashboard(フォーム)のパラメータ契約のため一元管理する。
 */
