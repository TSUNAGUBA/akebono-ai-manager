...
/** 投入ファイル名の規約(小文字英数字・ドット・ハイフン・アンダースコア+ .md / .txt)。 */
export const KNOWLEDGE_FILE_NAME_PATTERN = /^[a-z0-9_.-]+\.(md|txt)$/;
...
export function normalizeKnowledgeFileName(input: string): string {
  ...
  const withExt = /\.(md|txt)$/.test(trimmed) ? trimmed : `${trimmed}.md`;
  if (withExt.length > FILE_NAME_MAX_LENGTH) {
    throw invalidInput(`ファイル名は拡張子を含め ${FILE_NAME_MAX_LENGTH} 文字以内で入力してください`);
  }
  if (!KNOWLEDGE_FILE_NAME_PATTERN.test(withExt)) {
    throw invalidInput(
      'ファイル名は半角の小文字英数字・ドット・ハイフン・アンダースコア+拡張子 .md / .txt で指定してください(例: operations.md)',
    );
  }
  return withExt;
}
    ...
    <div class="form-grid" style="margin-top:14px">
      ...
      <label class="field">ファイル名
        <input type="text" name="file_name" required maxlength="128" pattern="[a-z0-9_.-]+"
               ...
               title="半角の小文字英数字・ドット・ハイフン・アンダースコア。拡張子は .md / .txt(省略時は .md を付与)">
      </label>
    </div>
...
export async function handleAdminKnowledgePost(
  ...
  if (action === 'delete') {
    ...
    auditLog(viewer, 'knowledge.delete', { fileId, fileName }, {});
