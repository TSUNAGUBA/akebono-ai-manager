...
SELECT format('CREATE ROLE dashboard_ro LOGIN PASSWORD %L', :'dashboard_ro_password')
WHERE NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'dashboard_ro')
\gexec

-- 接続権限
GRANT CONNECT ON DATABASE ai_manager TO app_rw, dashboard_ro;
